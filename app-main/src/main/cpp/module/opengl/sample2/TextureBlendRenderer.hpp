...
class TextureBlendRenderer : public GLRenderer {
private:
...
public:
    ...
    void onSurfaceCreated() override {
        ...
        texture = GlTexture::fromFile("image/head.png");
    }
    ...
    void onDrawFrame(void *data) override {
        program->startDraw([
            ...
            // 即使 PNG 带有透明度信息（alpha通道），OpenGL 默认不会自动处理透明效果。它会像处理不透明纹理一样，直接将
            // 新像素覆盖旧像素。
            // 启动混合，如果纹理有透明通道的话，需要启用混合。
            glEnable(GL_BLEND);
            // 指定如何混合。
            glBlendFunc(
                    GL_SRC_ALPHA,// 源的混合方式【即将要画的】
                    GL_ONE_MINUS_SRC_ALPHA// 目标的混合方式【已经画了的】
            );
